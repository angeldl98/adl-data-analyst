...
async function ensureDocsTable(client, target) {
...
async function publishDocs(client, rows, target) {
...
async function ensureMainTable(client, target) {
    await client.query(`CREATE SCHEMA IF NOT EXISTS ${target.schema}`);
    await client.query(`
...
async function upsertRows(client, target, rows, metaSchema, runId) {
    let processed = 0;
    let errors = 0;
...
async function materializeProduct(client, metaSchema, runId, activeRows, histRows) {
    let processed = 0;
    let errors = 0;
    ...
    try {
        ...
        const activeIds = activeRows.map((r) => r.subasta_id).filter((v) => v !== null);
        const histIds = histRows.map((r) => r.subasta_id).filter((v) => v !== null);
